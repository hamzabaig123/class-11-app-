...
export const appRouter = createTRPCRouter({
  ...
  notes: createTRPCRouter({
    ...
    linkQuestion: protectedProcedure.input(z.object({ noteId: z.string(), questionId: z.string() })).mutation(async ({ ctx, input }) => {
      const note = await prisma.note.findFirst({ where: { id: input.noteId, userId: ctx.user.id } }); if (!note) throw new TRPCError({ code: 'NOT_FOUND', message: 'Note not found' })
      const question = await prisma.question.findFirst({ where: { id: input.questionId, userId: ctx.user.id } }); if (!question) throw new TRPCError({ code: 'NOT_FOUND', message: 'Question not found' })
      const existing = await prisma.noteQuestion.findUnique({ where: { noteId_questionId: { noteId: input.noteId, questionId: input.questionId } } }); if (existing) throw new TRPCError({ code: 'CONFLICT', message: 'Already linked' })
      return prisma.noteQuestion.create({ data: { noteId: input.noteId, questionId: input.questionId } })
    }),
    unlinkQuestion: protectedProcedure.input(z.object({ noteId: z.string(), questionId: z.string() })).mutation(async ({ ctx, input }) => {
      await prisma.noteQuestion.deleteMany({ where: { noteId: input.noteId, questionId: input.questionId } }); return { success: true }
    }),
  }),
  ...
  analytics: createTRPCRouter({
    ...
    accuracyTrend: protectedProcedure.input(z.object({ from: z.string(), to: z.string(), bucket: z.enum(['daily', 'weekly']).optional(), timezone: z.string().optional() })).query(async ({ ctx, input }) => {
      ...
      const attempts = await prisma.attempt.findMany({ where: { userId, createdAt: { gte: from, lte: to } }, select: { isCorrect: true, createdAt: true } }); if (attempts.length === 0) return []
    ...
    recommendation: protectedProcedure.input(z.object({ from: z.string().optional(), to: z.string().optional(), timezone: z.string().optional() })).query(async ({ ctx, input }) => {
      const userId = ctx.user.id; const tz = input.timezone ?? await getUserTimezone(userId); const now = new Date(); const to = new Date(now.toLocaleString('en-US', { timeZone: tz })); to.setHours(0, 0, 0, 0); const from = new Date(to); from.setDate(from.getDate() - 30)
      const [due, attempts] = await Promise.all([prisma.reviewItem.count({ where: { userId, nextReviewAt: { lte: to }, status: { in: ['NEW', 'LEARNING', 'REVIEW', 'LAPSED'] } } }), prisma.attempt.findMany({ where: { userId, createdAt: { gte: from, lte: to } }, select: { isCorrect: true, questionId: true } })])
